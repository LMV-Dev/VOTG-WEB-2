...
export const getServerSideProps = async (context) => {
    ...
    try {
        const resAnswer = await axios
            ...
            // .get(`https://api.koaiarchitecture.com/online/survey/answer`, {
            //     params: { surveyCode: code },
            // })
            .get(`https://api.koaiarchitecture.com/online/survey/answer/eachurl`, {
                params: { eachUrl: c, surveyCode: code },
            })
        const resSurvey = await axios.get(`https://api.koaiarchitecture.com/online/survey/answer`, {
            params: { surveyCode: code },
        })
    ...
    const onSurveyComplete = async () => {
        try {
            if (surveyStatus == 0) {
                ...
                const res = await axios.post(`https://api.koaiarchitecture.com/online/survey/answer/eachurl`, {
                    eachUrl: c,
                    phoneCode: phoneCode,
                    surveyCode: code,
                    answerJson: answerData,
                    orderCode: rewadData ? rewadData.code : undefined,
                    productNumber: rewadData ? rewadData.item.NO_REQ : undefined,
                })
                // console.log(res)
                                ...
                                {surveyLoadData?.info.logoImage?.length > 0 && (
                                    <>
                                        ...
                                        >
                                            ...
                                            >
                                                <Image src={`https://api.koaiarchitecture.com/${surveyLoadData?.info?.logoImage}`} priority={true} layout="fill" objectFit="contain" objectPosition="left" />
                                            </div>
                                        </Box>
